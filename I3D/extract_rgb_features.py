...
i3d_rgb = InceptionI3d(in_channels=3,
                       name='i3d_rgb',
                       model_dir='/home/yangzehua/RoadAccidentsDetector/I3D/{}_imagenet.pt'.format('rgb')).eval().cuda()
...
def extract_rgb_features(videopath, mode='save'):
    ...
    if mode == 'save':
        torch.save(rgb_feat_tensor, os.path.join(feats_dir, split, '{}_rgb.pt'.format(video_name)))
    elif mode == 'output':

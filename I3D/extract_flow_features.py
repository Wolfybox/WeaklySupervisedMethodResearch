...
i3d_flow = InceptionI3d(in_channels=2,
                        name='i3d_flow',
                        model_dir='/home/yangzehua/RoadAccidentsDetector/I3D/flow_imagenet.pt'.format('flow')).eval().cuda()
...
def extract_flow_features(videopath, mode='save'):
    vname = videopath.split('/')[-1].split('.')[0]
    ...
    if mode == 'save':
        torch.save(flow_feat_tensor, os.path.join(feats_dir, split, '{}_flow.pt'.format(vname)))
    elif mode == 'output':
...
if __name__ == '__main__':
    ...
    feats_dir = '/home/yangzehua/UCF_Crimes/CADP_FLOW_Features'
    ...
    for vp in tqdm(videolist):
        try:
            print('Extracting Flow for {}'.format(vp))
            extract_flow_features(vp)
            finished.append(vp)
        except Exception:
            with open('log/flow_cadp.txt', 'w') as f:
                for fin in finished:
                    f.write('{}\n'.format(fin))
